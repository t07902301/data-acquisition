...
def method_run(n_img_list, acquisition_config:Config.Acquistion, checker: Checker.subset, data_split:Dataset.DataSplits, pdf_method):
    acc_change_list = []
...
def run(acquisition_config:Config.Acquistion, methods, new_img_num_list, checker:Checker.subset, data_split:Dataset.DataSplits, pdf_method):
    result_list = []
...
def epoch_run(new_img_num_list, method_list, acquire_instruction:Config.Acquistion, checker: Checker.subset, data_split:Dataset.DataSplits, pdf_method):
...
def main(epochs, new_model_setter='retrain', pure=False, model_dir ='', device=0, probab_bound = 0.5, base_type='', detector_name = ''):
    ...
    acquire_instruction = Config.AcquistionFactory('non_seq',seq_rounds_config) 
    ...
    for epo in range(epochs):
        ...
        checker = tools.get_probab_checker(epo, parse_para, ds_list[epo], stream_instruction, detect_instruction, plot=False)

...
def get_config(model_config:Config.NewModel, acquisition_config:Config.Acquistion, log_symbol):
    log_config = Config.Log(batch_size=model_config.batch_size,class_number=model_config.class_number,model_dir=model_config.model_dir,pure=model_config.pure,setter=model_config.setter,model_cnt=model_config.model_cnt, device=model_config.device, augment=model_config.augment, log_symbol=log_symbol)
...
def save(data, log_config:Config.Log):
    if log_config.log_symbol == 'data' and log_config.augment:
...
def get_log_data(acquisition_config:Config.Acquistion, model_config:Config.NewModel, dataset_splits:Dataset.DataSplits):
    ...
    log_config = get_config(model_config, acquisition_config, log_symbol)
    log_config.set_path(acquisition_config)
...
def get_log_clf(acquisition_config:Config.Acquistion, model_config:Config.NewModel):
    log_config = get_config(model_config, acquisition_config, 'clf')
    log_config.set_path(acquisition_config)
    clf = load(log_config)
    return clf
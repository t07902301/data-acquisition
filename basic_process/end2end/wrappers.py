...
class Dataset():
    ...
    def get_raw_indices(self, dataset:data.Dataset):
        '''
    ...
    @abstractmethod
    def load(self, ds_dict, data_config, normalized_stat):
        '''
        Load indices and get data splits with shifts from the raw data
        '''
        pass
    ...
    def get_basic_split_indices(self, dataset_labels, target_labels, split_1_ratio):
        '''
...
class Cifar(Dataset):
    ...
    def basic_split(self, dataset, labels, target_ratio, use_fine_label = True, return_split_indices=False):
        '''
        ...
        target_splits, other_splits = self.get_basic_split_indices(dataset_labels, labels, target_ratio)
    ...
    def load(self, raw_indices_dict:Dict[str, list], data_config, normalized_stat, sampled_meta_path=None):
    ...
    def create_shifts(self, ds_dict:Dict[str, cifar.CIFAR100], remove_rate, label_config):
        if 'cover' in label_config:
            return self.cover_shifts(ds_dict, remove_rate, label_config['cover'])
        else:
            return self.non_cover_shifts(ds_dict, remove_rate, label_config)

    def non_cover_shifts(self, ds_dict:Dict[str, cifar.CIFAR100], remove_rate, label_config):
        ...
        _, old_train = self.basic_split(ds_dict['train'], new_labels, new_labels_remove_rate)

        _, old_val = self.basic_split(ds_dict['val_shift'], new_labels, new_labels_remove_rate)

        _, old_test = self.basic_split(ds_dict['test_shift'], new_labels, new_labels_remove_rate)

        if old_labels_remove_rate != None:
            _, val = self.basic_split(ds_dict['val_shift'], old_labels, old_labels_remove_rate)
            _, test = self.basic_split(ds_dict['test_shift'], old_labels, old_labels_remove_rate)
    ...
    def cover_shifts(self, ds_dict:Dict[str, cifar.CIFAR100], remove_rate, cover_labels):

        new_labels_remove_rate = remove_rate['new_labels']
        old_labels_remove_rate = remove_rate['old_labels']
        logger.info('Cover Labels remove, src:{}, target: {}'.format(cover_labels['src'], cover_labels['target']))

        old_train, _ = self.basic_split(ds_dict['train'], cover_labels['src'], new_labels_remove_rate)
        old_val, _ = self.basic_split(ds_dict['val_shift'], cover_labels['src'], new_labels_remove_rate)
        old_test, _ = self.basic_split(ds_dict['test_shift'], cover_labels['src'], new_labels_remove_rate)
        val_shift, _ = self.basic_split(ds_dict['val_shift'], cover_labels['target'], new_labels_remove_rate)
        test_shift, _ = self.basic_split(ds_dict['test_shift'], cover_labels['target'], new_labels_remove_rate)
    ...
    def create_splits(self, config, raw_ds:Dict[str, cifar.data.Dataset]):
        ...
        train_ds, market_ds = self.basic_split(train_market, label_summary, train_size)
        ...
        test_val, _ = self.basic_split(test_val, label_summary, test_size)

        val_ds, test_ds = self.basic_split(test_val, label_summary, val_from_test)
    ...
    def create(self, epochs, config, meta_path):
        data_config = config['data']
        label_config = data_config['labels']
        select_fine_labels = label_config['select_fine_labels']
        ...
        for epo in range(epochs):
            split_dict = self.create_splits(config, raw_dataset)  
            logger.info('get data splits')

            shift_indices = self.create_shifts(split_dict, data_config['ratio']['remove'], label_config)   
            logger.info('get data shifts')

            raw_indices_list.append(shift_indices)
...
class Core(Dataset):
    ...
    def basic_split(self, dataset, target_ratio, option: dict, return_splits=False):
        '''
    ...
    def dual_operate(self, primal_dict, remove_rate, remove_label_config, option):
        ...
        return {
            ...
            'aug_market': self.get_raw_indices(primal_dict['aug_market']),
        }
    ...
    def create_shifts(self, ds_dict, remove_rate, label_config, option):
    ...
    def create_splits(self, data_config, meta: MetaData):
    ...
    def create(self, epochs, config, sampled_meta_path, option):
        data_config = config['data']
        sampled_meta = MetaData(sampled_meta_path)
        ...
        raw_indices_list = []
        for epo in range(epochs):
            split_dict = self.create_splits(data_config, sampled_meta)
            shift_dict = self.create_shifts(split_dict, data_config['ratio']['remove'], data_config['label'], option)
            raw_indices_list.append(shift_dict)

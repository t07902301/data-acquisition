...
def compare(acquisition_config:Config.Acquistion, model_config:Config.NewModel, product:Checker.subset, dataset_splits:Dataset.DataSplits, check_class):
    if model_config.pure:
        test_distribution = product.test_info['dv'][product.test_info['gt']==check_class]
        dv_result = {
...
def main(epochs, n_data, acquistion_class, new_model_setter='retrain', pure=False, model_dir ='', distribution_type='', device=0, augment=True):
    print('Use pure: ',pure)
    ...
    for epo in range(epochs):
        ...
        acquire_instruction.set_items('', n_data)
        dataset = ds_list[epo]
    ...
    plotter.run(epochs, results, distribution_type, acquistion_class, n_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-an','--acquistion_number',type=int, default=100)
    parser.add_argument('-ac','--acquistion_class',type=int, default=0)

    args = parser.parse_args()
    main(args.epochs, n_data=args.acquistion_number, pure=args.pure,model_dir=args.model_dir, distribution_type=args.distribution_type,device=args.device, augment=args.augment,acquistion_class=args.acquistion_class)
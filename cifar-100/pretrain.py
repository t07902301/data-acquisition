...
def main(epochs,  model_dir ='', train_flag=False, device=0):
    ...
    for epo in range(epochs):
        ...
        ds = ds_list[epo]
        dataset_splits = Dataset.DataSplits(ds, old_model_config.batch_size)
        acc, score, prec, shift_score, acc_shift = run(dataset_splits, old_model_config, train_flag)
        acc_list.append(acc)
        clf_score_list.append(score)
        clf_prec_list.append(prec)
        shift_score_list.append(shift_score)
        acc_shift_list.append(acc_shift)
        print('Model Acc before shift:', np.round(acc,decimals=3))
        print('Model Acc after shift:', np.round(acc_shift,decimals=3))
        print('Distribution Shift Proportion on Model Misclassifications', np.round(shift_score, decimals=3))
        print(score)
        print(prec)

    print('Average Acc before shift:', np.round(np.mean(acc_list),decimals=3))
    print('Average Acc after shift:', np.round(np.mean(acc_shift_list),decimals=3))

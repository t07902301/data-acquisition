...
def total(check_class, data_splits:Dataset.DataSplits, clf):
    ...
    for split_name in used_splits:
        ...
        print(split_name, np.max(split_dv[split_name]), np.min(split_dv[split_name]))
    return split_dv
...
def main(epochs, method, n_data, acquistion_class, new_model_setter='retrain', pure=False, model_dir ='', distribution_type='', device=0, augment=True):
    ...
    for epo in range(5):
        ...
        gts, pred, _ = Model.evaluate(dataset_splits.loader['val_shift'], base_model)
        val_shift_info, precision = clf.predict(dataset_splits.loader['val_shift'], compute_metrics=True, dataset_preds=pred)
        for c in range(old_model_config.class_number):
            print(np.max(val_shift_info['dv'][val_shift_info['gt']==c]), np.min(val_shift_info['dv'][val_shift_info['gt']==c]))
        print(precision, score)
    # for k in dataset.keys():
    #     print(k, len(dataset[k]))

    # for c in range(3):
    #     distribution = get_distribution(dataset_splits, acquire_instruction, new_model_config, product, c, distribution_type)
    #     results.append(distribution)
 
    # plotter = Plotter.Distribution(new_model_config)
    # plotter.run(3, results, distribution_type, acquistion_class, method, n_data)

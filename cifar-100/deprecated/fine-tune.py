...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, is_inception=False):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['test']:
            ...
            # Iterate over data.
            for inputs,labels,fine_labels in dataloaders[phase]:
...
ds = create_dataset_split(train_ds,aug_train_ds,test_ds,select_fine_labels=None)
subset_loader = {
...
#     outputs = model_ft(batch[0])
#     _, preds = torch.max(outputs, 1)
#     running_corrects += torch.sum(preds==batch[1])
# acc = running_corrects.double() / len(dataloaders_dict['val'].dataset)
...
# Train and evaluate
model_ft, hist = train_model(model_ft, subset_loader, criterion, optimizer_ft, num_epochs=num_epochs, is_inception=(model_name=="inception"))
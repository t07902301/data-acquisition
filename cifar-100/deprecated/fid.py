...
def fid(ds,batch_size,dims,num_workers,device):
    fid_value = calculate_fid_given_paths( ds,
                                          batch_size,
                                          device,
                                          dims,
                                          num_workers)
...
def calculate_fid_given_paths(ds, batch_size, device, dims, num_workers=1):
    ...
    m1, s1 = compute_statistics_of_path(ds[0], model, batch_size,
                                        dims, device, num_workers)
    m2, s2 = compute_statistics_of_path(ds[1], model, batch_size,
                                        dims, device, num_workers)
...
def get_activations(images, model, batch_size=50, dims=2048, device='cpu',
    ...
    if batch_size > img_len:
        print(('Warning: batch size is bigger than the data size. '
               'Setting batch size to data size'))
        batch_size = img_len

    # dataset = ImagePathDataset(images, transforms=TF.ToTensor())
    dataset = images
    ...
    pred_arr = np.empty((img_len, dims))

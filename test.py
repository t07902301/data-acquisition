...
def threshold_run(epochs, parse_args, dataset_list, new_img_num_list, method_list, operation: Config.Operation):
    results = []
    threshold_stat_list = []
    for epo in range(epochs):
        logger.info('in epoch {}'.format(epo))
        checker = Checker.instantiate(epo, parse_args, dataset_list[epo], operation) #probab / ensemble
        result_epoch, threshold_stat = epoch_run(new_img_num_list, method_list, operation, checker)
        results.append(result_epoch)
        threshold_stat_list.append(threshold_stat)
    return results, threshold_stat_list

def main(epochs, acquisition_method, device, detector_name, model_dir, ensemble_name, base_type, ensemble_criterion, utility_threshold, pdf= 'kde'):
    ...
    result, threshold_stat = threshold_run(epochs, parse_args, ds_list, config['data']['budget'], acquisition_method, operation)
...
if __name__ == '__main__':
    ...
    main(args.epochs, model_dir=args.model_dir, device=args.device, detector_name=args.detector_name, acquisition_method=args.acquisition_method, ensemble_name=args.ensemble, base_type=args.base_type, ensemble_criterion=args.criterion, utility_threshold=args.utility_threshold)
